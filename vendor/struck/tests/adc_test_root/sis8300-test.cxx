...
unsigned short* ushort_adc_buffer_ptr[10]; //  
...
//main(int argc, char *argv[]){
  ...
  unsigned int ad9510_divider_configuration_array[9]  ;
  ...
  ushort_adc_buffer_ptr[8] = gl_ushort_adc9_buffer;
  ushort_adc_buffer_ptr[9] = gl_ushort_adc10_buffer;
  ...
  /* Save command line into string "command" */
	...
	switch (ch) {
	  ...
	  default:
	    ...
	    printf("   \n");
	    printf("       -Z  	       Auto-Zoom \n");
	    printf("       -A num      Display Graph of ADC channel (default:0; 0=all channles, 1 to 10 = chx) \n");
	    ...
	    printf("       -C 3        External RTM Clock 0 and 1 \n");
	    printf("       -C 4        External RTM Clock 2 \n");
	    printf("   \n");
	...
	test_sample_mode = cmd_sample_mode ;
	 ...
	 test_sample_mode = 0 ;
      ...
      if ((cmd_clock_source >= 0) && (cmd_clock_source <= 4)) {
	test_clock_source = cmd_clock_source ;
	 ...
	 test_clock_source = 0 ;
      ...
      TGraph *gr9 = new TGraph(n,x,y);  
      TGraph *gr10 = new TGraph(n,x,y);  
      ...
      gr9->SetLineColor(Channel_9_Color);
      gr10->SetLineColor(Channel_10_Color);
      ...
      TLatex *text_adc9 = new TLatex(10,10,"Ch 9");
      TLatex *text_adc10 = new TLatex(10,10,"Ch 10");
      ...
      text_adc9->SetTextColor(Channel_9_Color);
      text_adc10->SetTextColor(Channel_10_Color);
	...
	switch(test_clock_source){
	  ...
	  case 2:
	    ...
	    break;
	  case 3:
	    myReg.data = 0x0050F; // external RTM clock 0 1 (no MUX setting necessary)
	    break;
	  case 4:
	    myReg.data = 0x00000; // external RTM clock 2
	    break;
	}
...
//ch_divider_configuration_array[i] :  (i = 0...7) 
...
//ch_divider_configuration_array[8] : Clock select and Power down options (SPI-addr. 45Hex)
//	bits <0>:     Select CLK IN (0 = CLK2, 1 = CLK1)
//	bits <1>:     CLK1 Power Down (1 = Power Down)
//	bits <2>:     CLK2 Power Down (1 = Power Down)
//	bits <3>:     Prescaler CLK Power Down (1 = Shut Down Clock Signal to PLL Prescaler)
//	bits <4>:     REFIN Power Down (1 = Power Down)
//	bits <5>:     All CLK Inputs Power Down (1 = Power Down)
//	bits <7:6>:   not used
...
//#define CLK_25MHZ //with Phase
...
#else
	ad9510_divider_configuration_array[0] = 0x0000 + 0x00 ;  // (ADC1-CLK, ch1/2)
	ad9510_divider_configuration_array[1] = 0x0000 + 0x00 ;  // (ADC2-CLK, ch3/4)
	ad9510_divider_configuration_array[2] = 0x0000 + 0x00 ;  // (ADC3-CLK, ch4/5)
	ad9510_divider_configuration_array[3] = 0x0000 + 0x00 ;  // (ADC4-CLK, ch6/7)
	ad9510_divider_configuration_array[4] = 0x0000 + 0x00 ;  // (ADC5-CLK, ch8/9)
	ad9510_divider_configuration_array[5] = 0x0000 + 0x00 ;  // (Frontpanel Clk, Harlink)
	ad9510_divider_configuration_array[6] = 0x0000 + 0x00 ;  // (FPGA DIV-CLK05) used for synch. of external Triggers
#endif
	ad9510_divider_configuration_array[7] = 0xC000 + 0x00 ;  // (FPGA DIV-CLK69) used for sychn. of AD9510 I´Sc and Bypass

      if (test_clock_source == 3) {
	 ad9510_divider_configuration_array[8] = 0x0000 + 0x1A ;  // (CLK2 Drives Distribution Section, Power-Down CLK1, RefIn, Clk to PLL Prescaler) 
      }
      else {
	 ad9510_divider_configuration_array[8] = 0x0000 + 0x1D ;  // (CLK1 Drives Distribution Section, Power-Down CLK2, RefIn, Clk to PLL Prescaler) 
      }
	...
	switch(test_adc_no){
	  case 1:
	    myReg.data = 0x3fe; // enable ch1
	    break;
	  case 2:
	    myReg.data = 0x3fd; // enable ch2
	    break;
	  case 3:
	    myReg.data = 0x3fb; // enable ch3
	    break;
	  case 4:
	    myReg.data = 0x3f7; // enable ch4
	    break;
	  case 5:
	    myReg.data = 0x3ef; // enable ch5
	    break;
	  case 6:
	    myReg.data = 0x3df; // enable ch6
	    break;
	  case 7:
	    myReg.data = 0x3bf; // enable ch7
	    break;
	  case 8:
	    myReg.data = 0x37f; // enable ch8
	    break;
	  case 9:
	    myReg.data = 0x2ff; // enable ch9
	    break;
	  case 10:
	    myReg.data = 0x1ff; // enable ch10
	    break;
	}
#endif	
    // if external trigger
...
#define SIS8300_TRIGGER_THRESHOLD_CH1_REG		0x110
	...
	do {
	      printf("dual_channel_memory_logic_flag = 0x%08X\n", dual_channel_memory_logic_flag);
		...
		sis8300_WriteRegister( fp, SIS8300_SAMPLE_START_ADDRESS_CH9_REG, 0x800000 ); // 9. 1M-Block 16 Msamples
		sis8300_WriteRegister( fp, SIS8300_SAMPLE_START_ADDRESS_CH10_REG, 0x900000 );// 10. 1M-Block 16 Msamples
	    }
	    else { //DUAL_CHANNEL_BUFFER_LOGIC
		sis8300_WriteRegister( fp, SIS8300_SAMPLE_START_ADDRESS_CH1_REG, 0x0 );      // 1. 2M-Block 16 Msamples
		sis8300_WriteRegister( fp, SIS8300_SAMPLE_START_ADDRESS_CH2_REG, 0x200000 ); // 2. 2M-Block 16 Msamples
		sis8300_WriteRegister( fp, SIS8300_SAMPLE_START_ADDRESS_CH3_REG, 0x400000 ); // 3. 2M-Block 16 Msamples
		sis8300_WriteRegister( fp, SIS8300_SAMPLE_START_ADDRESS_CH4_REG, 0x600000 ); // 4. 2M-Block 16 Msamples
		sis8300_WriteRegister( fp, SIS8300_SAMPLE_START_ADDRESS_CH5_REG, 0x800000 ); // 5. 2M-Block 16 Msamples
	    }
	 ...
	 // wait if BUSY or Arm
	    ...
	    memset(gl_ushort_adc9_buffer, 0, numSamples*2);
	    memset(gl_ushort_adc10_buffer, 0, numSamples*2);

// read all ADC channels
	    if (dual_channel_memory_logic_flag == 0) { //SINGLE_CHANNEL_BUFFER_LOGIC
		for (i_ch=0; i_ch<max_nof_channels; i_ch++) {
#ifdef raus
		    myReg.offset = DMA_READ_SRC_ADR_LO32;
		    myReg.data = (i_ch) * (0x100000 * 16 * 2); // Blocklength * 16 Samples/Block * 2Byte/Sample
		    //printf("offs adc%d ch1:%08X\n", test_adc_no, myReg.data);
		    ioctl(fp, SIS8300_REG_WRITE, &myReg);
		    //read(fp, (uint32_t *) gl_ushort_adc1_buffer, numSamples*2);
		    read(fp, (uint32_t *) ushort_adc_buffer_ptr[i_ch], numSamples*2);
#endif
		    sis8300_ReadMemory(fp, (i_ch) * (0x100000 * 16 * 2), numSamples*2, (uint32_t *) ushort_adc_buffer_ptr[i_ch]);
		}
	    }
	    else { //DUAL_CHANNEL_BUFFER_LOGIC
		for (i_ch=0; i_ch<max_nof_channels; i_ch=i_ch+2) {
#ifdef raus
		    myReg.offset = DMA_READ_SRC_ADR_LO32;
		    myReg.data = (i_ch) * (0x100000 * 16 * 2); // Blocklength * 16 Samples/Block * 2Byte/Sample
		    //printf("offs adc%d ch1:%08X\n", test_adc_no, myReg.data);
		    ioctl(fp, SIS8300_REG_WRITE, &myReg);
		    //read(fp, (uint32_t *) gl_ushort_adc1_buffer, numSamples*2);
		    read(fp, (uint32_t *) gl_uint_adc_buffer, numSamples*4);
#endif
		    // addr= Blocklength * 16 Samples/Block * 2Byte/Sample
		    sis8300_ReadMemory(fp, (i_ch) * (0x100000 * 16 * 2), numSamples*4, gl_uint_adc_buffer);
   
		    ushort_adcA_buffer_ptr = ushort_adc_buffer_ptr[i_ch] ; //  
		    ushort_adcB_buffer_ptr = ushort_adc_buffer_ptr[i_ch+1]  ; //  

		    for(ui = 0; ui < numSamples;ui++){
			ushort_adcA_buffer_ptr[ui] = (unsigned short) gl_uint_adc_buffer[ui] & 0xffff;
			ushort_adcB_buffer_ptr[ui] = (unsigned short) (gl_uint_adc_buffer[ui] >> 16) & 0xffff;
		    }
		}
	    }
...
//	      printf("adc%2d min:%u max:%u ampl:%4u -3dB:%5.1f\n", i+1, gl_adc_min[i], gl_adc_max[i], gl_adc_max[i] - gl_adc_min[i], (gl_adc_max[i] - gl_adc_min[i]) * 0.7);
	  ...
	  // check min and max for y-cordiante
		...
		if ((test_adc_no == 0) || (test_adc_no == 2)) {
		  for(i=0; i<numSamples; i++){
		...
		if ((test_adc_no == 0) || (test_adc_no == 4)) {
		  for(i=0; i<numSamples; i++){
		...
		if ((test_adc_no == 0) || (test_adc_no == 6)) {
		  for(i=0; i<numSamples; i++){
		...
		if ((test_adc_no == 0) || (test_adc_no == 8)) {
		  for(i=0; i<numSamples; i++){
		...
		if ((test_adc_no == 0) || (test_adc_no == 9)) {
		  for(i=0; i<numSamples; i++){
		    if(gl_ushort_adc9_buffer[i] < ymin) ymin = gl_ushort_adc9_buffer[i];
		    if(gl_ushort_adc9_buffer[i] > ymax) ymax = gl_ushort_adc9_buffer[i];
		  }
		}

		if ((test_adc_no == 0) || (test_adc_no == 10)) {
		  for(i=0; i<numSamples; i++){
		    if(gl_ushort_adc10_buffer[i] < ymin) ymin = gl_ushort_adc10_buffer[i];
		    if(gl_ushort_adc10_buffer[i] > ymax) ymax = gl_ushort_adc10_buffer[i];
		  }
		}
	...
	// initialze x-cordinates
	  ...
	  // plot channel 9
	    if ((test_adc_no == 0) || (test_adc_no == 9)) {
		for (i=0; i<numSamples; i++){
		    y[i] = (int) gl_ushort_adc9_buffer[i]; 
		}
		gr9-> DrawGraph(numSamples,x,y, "L");   
		text_adc9->DrawLatex(numSamples+10,ymax-(9*y_delta),"Ch 9");
	    }

	  // plot channel 10
	    if ((test_adc_no == 0) || (test_adc_no == 10)) {
		for (i=0; i<numSamples; i++){
		    y[i] = (int) gl_ushort_adc10_buffer[i]; 
		}
		gr10-> DrawGraph(numSamples,x,y, "L");   
		text_adc10->DrawLatex(numSamples+10,ymax-(10*y_delta),"Ch 10");
	    }
	...
	if( cmd_dump_loops ){
	  if( dump_loop_counter < cmd_dump_loops ){
	    ...
	    if ((test_adc_no == 0) || (test_adc_no == 2)) dumpSamplesToFile( 2, dump_loop_counter, gl_ushort_adc2_buffer, numSamples );
	    if ((test_adc_no == 0) || (test_adc_no == 3)) dumpSamplesToFile( 3, dump_loop_counter, gl_ushort_adc3_buffer, numSamples );
	    if ((test_adc_no == 0) || (test_adc_no == 4)) dumpSamplesToFile( 4, dump_loop_counter, gl_ushort_adc4_buffer, numSamples );
	    if ((test_adc_no == 0) || (test_adc_no == 5)) dumpSamplesToFile( 5, dump_loop_counter, gl_ushort_adc5_buffer, numSamples );
	    if ((test_adc_no == 0) || (test_adc_no == 6)) dumpSamplesToFile( 6, dump_loop_counter, gl_ushort_adc6_buffer, numSamples );
	    if ((test_adc_no == 0) || (test_adc_no == 7)) dumpSamplesToFile( 7, dump_loop_counter, gl_ushort_adc7_buffer, numSamples );
	    if ((test_adc_no == 0) || (test_adc_no == 8)) dumpSamplesToFile( 8, dump_loop_counter, gl_ushort_adc8_buffer, numSamples );
	    if ((test_adc_no == 0) || (test_adc_no == 9)) dumpSamplesToFile( 9, dump_loop_counter, gl_ushort_adc9_buffer, numSamples );
	    if ((test_adc_no == 0) || (test_adc_no == 10)) dumpSamplesToFile( 10, dump_loop_counter, gl_ushort_adc10_buffer, numSamples );
...
// AD9510 (ADC channels 1 to 6)
	...
	myReg.data = myReg.data + (ch_divider_configuration_array[8] & 0xff) ; //  
	ioctl(device, SIS8300_REG_WRITE, &myReg);
	usleep(1) ;
...
#ifdef raus
struct SIS8300_ADC_SPI_Config_Struct {
...
int SIS8300_ADC_SPI_Setup(int device,  unsigned int adc_device_no, struct SIS8300_ADC_SPI_Config_Struct* sis3305_ADC_SPI_configuration_struct)
{
	unsigned int addr, data, read_loop;
	sis3305_ADC_SPI_configuration_struct->uintChipID[0] = data ;							 
	data = sis3305_ADC_SPI_configuration_struct->uintControl[0] & 0xffff ;
	return 0;			 
}
#endif 
