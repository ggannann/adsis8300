...
private:
   ...
   TGButton            *fOkButton,  *fStartB, *fStopB;
      ...
      : TGMainFrame(p, w, h)
{
  ...
  // Create test main frame. A TGMainFrame is a top level window.
   ...
   fMenuDock->SetWindowName("SIS8300 Test Menu");
...
// add SIS8300
   ...
   fMenuSIS8300Test->AddLabel("SIS8300 Tests");
   ...
   fMenuBar->AddPopup("&SIS8300-Test", fMenuSIS8300Test, fMenuBarItemLayout);
...
// search for sis8300 devices
   ...
   this->SetWindowName("SIS8300 Test (13.03.2013)");
...
// histograms
  ...
  // tf->AddFrame(fF2, fL3);
   ...
   fCombo_Display_Histos->Select(11, kTRUE); // display all
...
/**********/   
   ...
   //CenterOnParent();
   SetWindowName("SIS8300 Test Dialog");
   MapWindow();
...
// Parameter Setup
  ...
  if (sample_length > MAX_ROOT_PLOT_LENGTH) { sample_length = MAX_ROOT_PLOT_LENGTH - 15; }
...
// Raw Data Graph Setup
  ...
  if (display_histogram_choice > 0)  {
	...
	if (fChkHistoLogY->IsOn() == kTRUE)  { fChkHistoLogY_old_flag = 1; }
...
/***************************************************************************************************/
	...
	if (new_display_histogram_choice > 0)  {
		...
		for(i = 0; i < i_histo_ch; i++){
			...
			if(fChkHistoZoomMean->IsOn() == kTRUE)  {
				...
				if (double_histo_mean > 20.0) { double_histo_min_x = double_histo_mean - 20.0;}
				//if (double_histo_mean < 65515.0) { double_histo_max_x = double_histo_mean + 20.0;}
				if (double_histo_mean < (double) (i_histo_length-20)) { double_histo_max_x = double_histo_mean + 20.0;}
	...
	// display histograms  
		...
		if (fChkHistoLogY->IsOn() == kTRUE)  { fChkHistoLogY_new_flag = 1; }
		   ...
		   //fCanvas2->Divide(2,5);
			for (i=0;i<10;i++) {
				...
				if (fChkHistoLogY_changed_flag == 1) {
					if (fChkHistoLogY->IsOn() == kTRUE)  { gPad->SetLogy(1); }
					  else {  gPad->SetLogy(0);}
				}
